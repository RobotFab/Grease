...
- Default sketch location: `~/Documents/Grease/grease-X/grease-X.ino` (X = next available number). The **folder name must match the `.ino` filename** — this is an arduino-cli requirement. Always create sketches inside `~/Documents/Grease/`. Remind the user of the path after creating the sketch.
